...
if (binName != null) {
    window.addEventListener('DOMContentLoaded', function () {
        ...
        // Sự kiện khi nhấn vào nút "Lưu"
        document.getElementById('Save_model').addEventListener('click', function () {
            console.log(modelId);
            var cameraState = {
                Id: modelId,
                alpha: camera.alpha,
            ...
            saveCameraStateToServer(cameraState);
        });

        // Hàm gửi trạng thái camera lên server
        function saveCameraStateToServer(cameraState) {
            var xhr = new XMLHttpRequest();
            var url = 'admin/api/save_camera_state.php'; // Đường dẫn tới file PHP xử lý lưu trạng thái camera
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        console.log('Trạng thái camera đã được lưu thành công');
                    } else {
                        console.log('Đã xảy ra lỗi khi lưu trạng thái camera');
                    }
                }
            };

            xhr.send(JSON.stringify(cameraState));
        }
